  ...
  showResults = () => {
    ...
    this.questions.forEach((currentQuestion, questionNumber) => {
      ...
      const userAnswers = Array.from(
        answerContainer.querySelectorAll(selector)
      );

      // Get the values of all checked answers and sort them
      const userAnswerString = userAnswers
        .map((answer) => answer.value)
        .sort()
        .join("");

      if (userAnswerString === currentQuestion.correctAnswer) {
        numCorrect++;
